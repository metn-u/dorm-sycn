        ...
        <div className="space-y-12">
            <header className="flex items-end justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Manage shared expenses</p>
                </div>
                <div className={cn(
                ...
                <>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded-xl border border-rose-100 dark:border-rose-900/30">
                                <ArrowDownLeft className="w-5 h-5" strokeWidth={2.5} />
                            </div>
                            <h3 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Money You Owe</h3>
                            <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800 ml-3"></div>
                        </div>

                        {debtsIOwe.length === 0 ? (
                            <div className="bento-card bg-slate-50/50 dark:bg-slate-900/30 border-dashed border-2 border-slate-200 dark:border-slate-800 text-center py-12">
                                <p className="text-slate-400 dark:text-slate-500 font-bold italic">You're all settled! 🎉</p>
                            </div>
                        ) : (
                            <div className="grid gap-4">
                                {debtsIOwe.map(e => (
                                    <div key={e.id} className="bento-card flex items-center justify-between hover:bg-rose-50/30 dark:hover:bg-rose-900/10">
                                        <div className="flex items-center gap-4">
                                            <div className="p-3 bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded-2xl border border-rose-100 dark:border-rose-900/30">
                                                <Receipt className="w-6 h-6" strokeWidth={2} />
                                            </div>
                                            <div>
                                                <h4 className="font-bold text-slate-900 dark:text-white tracking-tight text-lg leading-none">{e.description}</h4>
                                        ...
                                        <div className="text-right flex flex-col items-end gap-3">
                                            <span className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">₺{calculateMyShareIOwe(e).toFixed(0)}</span>
                                            <button
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-xl border border-emerald-100 dark:border-emerald-900/30">
                                <ArrowUpRight className="w-5 h-5" strokeWidth={2.5} />
                            </div>
                            <h3 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Owed To You</h3>
                            <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800 ml-3"></div>
                        </div>

                        {debtsOwedToMe.length === 0 ? (
                            <div className="bento-card bg-slate-50/50 dark:bg-slate-900/30 border-dashed border-2 border-slate-200 dark:border-slate-800 text-center py-12">
                                <p className="text-slate-400 dark:text-slate-500 font-bold italic">Nothing here yet</p>
                            </div>
                        ) : (
                            <div className="grid gap-4">
                                {debtsOwedToMe.map(e => (
                                    <div key={e.id} className="bento-card flex items-center justify-between hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10">
                                        <div className="flex items-center gap-4">
                                            <div className="p-3 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                                                <Receipt className="w-6 h-6" strokeWidth={2} />
                                            </div>
                                            <div>
                                                <h4 className="font-bold text-slate-900 dark:text-white tracking-tight text-lg leading-none">{e.description}</h4>
                                        ...
                                        <div className="text-right flex flex-col items-end gap-3">
                                            <span className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">₺{calculateOthersShareOwedToMe(e).toFixed(0)}</span>
                                            <div className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-3 py-1 font-bold uppercase text-[9px] tracking-widest rounded-lg border border-slate-200 dark:border-slate-700">
                                                Awaiting payment
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Transaction Log</h3>
                            <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800 ml-3"></div>
                        </div>
                            ...
                            {allExpenses.map(expense => (
                                <div key={expense.id} className="p-4 flex items-center gap-4 relative group">
                                    <div className={cn(
                                        ...
                                        expense.status === 'paid'
                                            ? "bg-slate-50 dark:bg-slate-900/50 text-slate-300 dark:text-slate-600 border-slate-100 dark:border-slate-800"
                                            : "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-900/30 shadow-sm"
                                    )}>
                                        <Receipt className="w-6 h-6" strokeWidth={2} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-3">
                                            <h4 className={cn(
                                                ...
                                                expense.status === 'paid' ? "text-slate-400 dark:text-slate-600" : "text-slate-900 dark:text-white"
                                            )}>{expense.description}</h4>
                                            {expense.type === 'direct' && (
                                                <span className="text-[8px] bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-2 py-0.5 font-bold uppercase tracking-widest rounded">
                                                    Direct
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex flex-col items-end gap-1">
                                        <span className={cn(
                                            ...
                                            expense.status === 'paid' ? "text-slate-300 dark:text-slate-700" : "text-slate-900 dark:text-white"
                                        )}>₺{expense.amount.toFixed(0)}</span>
                                        {expense.status === 'paid' && (
                                            <span className="text-[8px] font-bold uppercase text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded border border-emerald-100 dark:border-emerald-900/30">
                                                Settled
                                            </span>
                                        )}
                                    </div>
